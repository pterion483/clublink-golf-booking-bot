#!/usr/bin/env python3
"""
Overnight Gap Finder - Claude Code Version
This script creates prompts for Claude Code to find and book morning tee times.
Run every hour overnight via cron to check for openings 2-5 days ahead.
Skips tomorrow to give more notice time.
"""
...
def setup_logging():
    ...
    log_file = os.path.join(LOG_DIR, f"gap_finder_overnight_{datetime.now().strftime('%Y%m%d')}.log")
...
def generate_claude_prompt():
    ...
    # Calculate date range - skip tomorrow, start from day 2
    today = datetime.now()
    dates_to_check = []
    for i in range(2, DAYS_AHEAD + 1):  # Start from day 2 to skip tomorrow
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Overnight Gap Finder Starting (Days 2-5)...")
